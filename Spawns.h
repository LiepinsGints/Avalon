...
{
public:
	Spawns(Ogre::Root* mRoot, Ogre::RenderWindow* mWindow, Ogre::SceneManager* mSceneMgr, PhysicsManager* physicsManager, MySql * mySql){
		_mRoot = mRoot;
		_mWindow = mWindow;
	...
	void createObject(Ogre::String meshName,Ogre::Vector3 position, float scale, Ogre::Real mass, int type ) {
		Models * model = _mySql->getModelByName(meshName);
		//Box descr
		NxOgre::BoxDescription boxDesc(scale*model->getDimensions().x, scale*model->getDimensions().y, scale*model->getDimensions().z);
		boxDesc.mGroup = type;
		//Body descr
